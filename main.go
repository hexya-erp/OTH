...
go-dest-dir:     target directory for the Hexya module`)
	...
	for _, c := range content {
		...
		if c.IsDir() {
			...
			case "report":
				reportDir := filepath.Join(sourceDir, "report")
				fileswiz, _ := ioutil.ReadDir(reportDir)
				for _, wiz := range fileswiz {

					if wiz.Name()[len(wiz.Name())-2:] == "py" {

						read, errr := ioutil.ReadFile(filepath.Join(reportDir, wiz.Name()))
						if errr != nil {
							fmt.Print(errr)
						}
						...
						doc := etree.NewDocument()
						if err := doc.ReadFromFile(filepath.Join(reportDir, wiz.Name())); err != nil {
							fmt.Print(err)
						}

						xml := translate.TransXML(doc, packageName)

						xml.WriteToFile(filepath.Join(destDir, "resources", "report_"+wiz.Name()))
					}
				}
