...
pool.ConfigParameter().DeclareTransientModel()
pool.ConfigParameter().AddBooleanField("CompanyShareProduct", models.SimpleFieldParams{String :"Share product to all companies" ,Help :"Share your product to all companies defined in your instance.\n  * Checked : Product are visible for every company, even if a company is defined on the partner.\n  * Unchecked : Each company can see only its product "})
...
func (rs pool.ConfigParameterSet , args struct{Fields interface{}
}){
  //@api.model
  /*def get_default_company_share_product(self, fields):
    ...
    */})
pool.ConfigParameter().Methods().SetAuthCompanyShareProduct().DeclareMethod(
`SetAuthCompanyShareProduct` ,
func (rs pool.ConfigParameterSet){
  //@api.multi
